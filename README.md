...
- **CI** — `.github/workflows/ci.yml` runs four parallel jobs on every push and PR: the backend suite (unit tests only on a PR, for a fast gate; the full suite, including the Testcontainers integration tests — GitHub-hosted runners have Docker available by default — on a push to `main`, via `./mvnw verify`), test + build for `frontend/` and `admin-portal/`, and type-check + Metro bundle for `mobile/`. Every job was run locally against the tree it was added to, so the pipeline started green rather than being debugged in CI.
