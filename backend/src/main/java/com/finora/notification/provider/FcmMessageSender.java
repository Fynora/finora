...
public interface FcmMessageSender {
     ...
     *
     * @param type {@link com.finora.notification.domain.NotificationType#name()} -- carried as a
     *             plain string, not the enum itself, so this seam stays decoupled from the domain
     *             package the same way {@code title}/{@code body} already are. Sent as FCM data
     *             (not part of the displayed notification), read by the mobile client to route a
     *             tap to the right screen -- see {@link FirebaseFcmMessageSender} for where it's
     *             attached to the outgoing {@code Message}.
     ...
     */
    FcmSendOutcome send(String deviceToken, String title, String body, String type);
}
