...
(matching `package_name: com.fynora.android`, `android.package` in `app.config.ts` — a different
identifier from iOS's `com.fynora.app`, see that file's own comment) to let the native build
proceed — it never validates
